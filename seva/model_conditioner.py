...
class SevaConditioner(pl.LightningModule):
    def __init__(self, ae, conditioner, version_dict, encoding_t):
        super().__init__()
        self.ae = ae
        self.conditioner = conditioner
        self.H, self.W, self.T, self.C, self.F, self.options = (
            version_dict["H"],
            version_dict["W"],
            version_dict["T"],
            version_dict["C"],
    ...
    def forward(self, value_dict) -> Dict:
        imgs = value_dict["cond_frames"].to("cuda")
        input_masks = value_dict["cond_frames_mask"].to("cuda")
        source_masks = value_dict["source_view_mask"].view(input_masks.shape).to("cuda")
        valid_masks = torch.logical_and(input_masks, torch.logical_not(source_masks))
        pluckers = value_dict["plucker_coordinate"].to("cuda")

        num_samples = [imgs.shape[0]//self.T, self.T]
        latents = torch.nn.functional.pad(
            self.ae.encode(imgs[input_masks], self.encoding_t), (0, 0, 0, 0, 0, 1), value=1.0
        )
        c_crossattn = self.conditioner(imgs[valid_masks])
        c_crossattn = c_crossattn.view(num_samples[0], -1, c_crossattn.shape[-1]).mean(1,keepdim=True)
        c_crossattn = repeat(c_crossattn, "bs 1 d -> bs n d", n=self.T).flatten(0,1)[:,None]
        uc_crossattn = torch.zeros_like(c_crossattn)
        c_replace = torch.zeros(imgs.shape[0], *latents.shape[1:]).type_as(latents)
        ...
        uc_concat = torch.cat(
            [pluckers.new_zeros(pluckers.shape[0], 1, *pluckers.shape[-2:]), pluckers], 1
        )
        ...
        additional_model_inputs = {"num_frames": self.T}
    ...
    def text_transformer_forward(self, x: torch.Tensor, attn_mask=None):
        ...
        for i, r in enumerate(self.model.transformer.resblocks):
            if i == len(self.model.transformer.resblocks) - 1:
                outputs["penultimate"] = x.permute(1, 0, 2).contiguous()  # LND -> NLD
            if (
        ...
        outputs["last"] = x.permute(1, 0, 2).contiguous()  # LND -> NLD
        return outputs

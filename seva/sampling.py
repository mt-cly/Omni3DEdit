...
class DiscreteDenoiser(object):
    ...
    def __call__(
        ...
        **additional_model_inputs,
        ...
        self.sigmas = self.sigmas.type(input.dtype).to(input.device)
        ...
        if "replace" in cond:
            # =======================todo liyi it is important to not pop since denoiser will be called multiple times
            # x, mask = cond.pop("replace").split((input.shape[1], 1), dim=1)
            x, mask = cond["replace"].split((input.shape[1], 1), dim=1)
            # ===============================
...
class DiscreteDenoiser_scaled_source(object):
    sigmas: torch.Tensor
    ...
    def __call__(
        ...
        **additional_model_inputs,
        ...
        if "replace" in cond:
            x, mask = cond.pop("replace").split((input.shape[1], 1), dim=1)
            input = input * (1 - mask) + x * mask
        if "concat_source" in cond:
            cond["concat"] = torch.cat([cond.pop("concat"), cond.pop("concat_source") * c_in], dim=1)
        return (
            network(input * c_in, c_noise, cond, **additional_model_inputs) * c_out
            + input * c_skip
        )

class DiscreteDenoiser_v5_singlestep_noresidual(object):
    sigmas: torch.Tensor
    ...
    def __call__(
        ...
        **additional_model_inputs,
        ...
        if "replace" in cond:
            ...
            input = input * (1 - mask) + x * mask
        return (
            network(input , c_noise, cond, **additional_model_inputs) 
            # + input * c_skip
        )

class DiscreteDenoiser_v10_x0pred(object):
    sigmas: torch.Tensor
...
class DiscreteDenoiser_v12(object):
    sigmas: torch.Tensor
    ...
    def update_c_concat(self, sourceview_intensity_mlp, c_concat, bs, num_f, num_sourceview, sigmas ):
        num_sourceview = num_sourceview//bs
        c_concat = c_concat.view(bs, num_f, *c_concat.shape[1:]) # [bs, num_f, 7, h, w]
        _sigmas = sigmas.view(bs, num_f, 1, 1, 1)
        c_concat[:, :num_sourceview, 0:1] = sourceview_intensity_mlp(_sigmas)[:, :num_sourceview]
        return c_concat.flatten(0,1)
    ...
    def __call__(
        ...
        **additional_model_inputs,
        ...
        # update concat set sigmas-awared sourceview intensity in concat
        assert "concat" in cond and "replace" in cond and "num_frames" in additional_model_inputs
        num_f = additional_model_inputs["num_frames"]
        bs = len(input) // num_f
        _, mask = cond["replace"].split((input.shape[1], 1), dim=1)
        num_sourceview = num_targetview = int((1-mask).mean((1,2,3)).sum())
        cond['concat'] = self.update_c_concat(network.module.source_view_intensity_mlp, cond['concat'], bs, num_f, num_sourceview, sigma)
...
class DiscreteDenoiser_v14(object):
    sigmas: torch.Tensor
    ...
    def update_c_concat_v14(self, c_concat,  source_view_mask):
        """Update concat for v14: set source view mask first channel to -1"""
        target_slice = c_concat[:, 0:1, :, :]
        new_data = torch.where(source_view_mask==1, -1, target_slice)
        c_concat[:, 0:1,:,:] = new_data    
        return c_concat.contiguous()
    ...
    def __call__(
        ...
        **additional_model_inputs,
        ...
        # update concat set source view mask first channel to -1
        assert "concat" in cond and "replace" in cond and "num_frames" in additional_model_inputs
        num_f = additional_model_inputs["num_frames"]
        bs = len(input) // num_f
        _, mask = cond["replace"].split((input.shape[1], 1), dim=1)
        source_view_mask = additional_model_inputs['source_view_mask']
        cond['concat'] = self.update_c_concat_v14(cond['concat'], source_view_mask)
...
class DiscreteDenoiser_v6_singlestep_residual(object):
    sigmas: torch.Tensor
...
class MultiviewScaleRule(object):
    ...
    def __call__(
        self,
        scale: float | torch.Tensor |tuple[float, float],
        c2w: torch.Tensor,
        K: torch.Tensor,
        ...
        if isinstance(scale, torch.Tensor):
            ...
            scale = torch.where(close_frame, scale[0], scale[1])
        else:
...
class ConstantScaleSchedule(object):
    def __call__(
        self, sigma: float | torch.Tensor, scale: float | torch.Tensor |tuple[float, float]
    ) -> float | torch.Tensor:
...
class VanillaCFG(object):
    ...
    def prepare_inputs(
        ...
        for k in c:
            if k in ["vector", "crossattn", "concat", "replace", "dense_vector", "source_latent"]:
...
class MultiviewCFG_uc_no_sourceview(VanillaCFG):
    def __init__(self, cfg_min: float = 1.0):
        self.scale_min = cfg_min
        self.scale_rule = MultiviewScaleRule(min_scale=cfg_min)
        self.scale_schedule = ConstantScaleSchedule()
    ...
    def prepare_inputs(
        ...
        for k in c:
            if k in ["vector", "crossattn", "concat", "replace", "dense_vector", "source_latent"]:
                ...
                assert any(c[k] == uc[k])
                c_out[k] = c[k].clone()
        # ========= 临时测试，由于当前uc denoise时，输入信息包括source_views+cond view+target views,这导致source_view产生一定的干扰
        # ========= 把uc部分对应的x输入中的sourcce_views用target views替换掉
        x_uc = x.clone()
        x_uc[:10] = x_uc[-10:].clone()
        return torch.cat([x_uc, x]), torch.cat([s] * 2), c_out
        # =====================================================
...
class MultiviewTemporalCFG(MultiviewCFG):
    ...
    def __call__(
        ...
        x: torch.Tensor,
        sigma: float | torch.Tensor,
        scale: float | torch.Tensor | tuple[float, float]| tuple[torch.Tensor, torch.Tensor],
        c2w: torch.Tensor ,
        K: torch.Tensor,
        input_frame_mask: torch.Tensor,
    ) -> torch.Tensor:
        ...
        if isinstance(scale, tuple):
            scale_min, scale = scale
        scale = min_distance * (scale - scale_min) + scale_min
...
class EulerEDMSampler(object):
    ...
    def mixed_denoise_zeroshot(
        ...
        cond: dict,
        uc: dict | None = None,
        zeroshot_cond: dict | None = None,
        zeroshot_uc: dict | None = None,
        num_steps: int | None = None,
        verbose: bool = True,
        return_intermediate: bool = False,
        **guider_kwargs,
        ...
        for i in self.get_sigma_gen(num_sigmas, verbose=verbose):
            ...
            if i < int(total_steps * 0.8):
                used_denoiser = denoiser
                used_cond = cond
                used_uc = uc
            else:
                used_denoiser = zeroshot_denoiser
                used_cond = zeroshot_cond
                used_uc = zeroshot_uc 
                scale = (1.2, 2.0)  # fixed scale for zeroshot denoiser
    ...
    def add_noise_and_sample(
        ...
        cond: dict,
        uc: dict | None = None,
        num_noise_steps: int = 20,
        verbose: bool = True,
        **guider_kwargs,
    ) -> torch.Tensor:
        # Verify that the original number of steps is specified
        if self.num_steps is None:
            raise ValueError("num_steps must be specified for the original sampler")
        
        # Generate original sigma schedule
        original_sigmas = self.discretization(self.num_steps, device=self.device)
        
        # Validate noise steps
        if num_noise_steps >= len(original_sigmas):
            raise ValueError(f"num_noise_steps ({num_noise_steps}) exceeds total steps ({len(original_sigmas)})")
        
        # Get initial sigma and truncated schedule
        sigma_initial = original_sigmas[num_noise_steps]
        new_sigmas = original_sigmas[num_noise_steps:]
        num_new_sigmas = len(new_sigmas)
        
        # Add noise to x0 using EDM parameterization
        epsilon = torch.randn_like(x0)
        x_t = x0 + sigma_initial * epsilon
        # x_t = epsilon * torch.sqrt(1.0 + sigma_initial ** 2)
        ...
        # Execute modified sampling loop
        for i in self.get_sigma_gen(num_new_sigmas, verbose=verbose):
            # Calculate gamma for stochastic sampling
            gamma = (
                min(self.s_churn / (len(original_sigmas) - 1), 2**0.5 - 1)
                if self.s_tmin <= new_sigmas[i] <= self.s_tmax
                else 0.0
            )
            
            # Perform sampler step
            x_t = self.sampler_step(
                s_in * new_sigmas[i],
                s_in * new_sigmas[i + 1],
                denoiser,
                x_t,
    ...
    def add_noise_and_sample_singlepass(
        self,
        denoiser,
        x0: torch.Tensor,
        scale: float | torch.Tensor,
        ...
        **guider_kwargs,
        ...
        # Calculate gamma for stochastic sampling
        gamma = (
            min(self.s_churn / (num_new_sigmas - 1), 2**0.5 - 1)
            if self.s_tmin <= new_sigmas[0] <= self.s_tmax
            else 0.0
        )
        
        # Perform sampler step
        x_0 = self.sampler_step(
            s_in * new_sigmas[0],
            s_in * new_sigmas[-1],
            denoiser,
            x_t,
    ...
    def get_max_sigma(self, n_samples):
        original_sigmas = self.discretization(self.num_steps, device=self.device)
        return original_sigmas[-4] * torch.ones((n_samples,)).to(self.device)
    ...
    def single_step(
        ...
        **guider_kwargs,
        ...
        # Get initial sigma and truncated schedule
        sigma_T = original_sigmas[0]
        sigma_0 = original_sigmas[-1]
        num_sigmas = len(original_sigmas)
        
        # Add noise to x0 using EDM parameterization

        x_t = x0 * torch.sqrt(1.0 + sigma_T ** 2)
        ...
        gamma = (
            min(self.s_churn / (num_sigmas - 1), 2**0.5 - 1)
            if self.s_tmin <= sigma_T <= self.s_tmax
            else 0.0
        )
